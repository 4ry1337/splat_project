...
public class SplatTester {
  ...
  private void runTest(File testFile) throws Exception {
    ...
    File progOutput = new File(testDir, testFile.getName() + "-out.txt");
    ...
    if (expectedResultCode < 4) {
      ...
      if (expectedResultCode == actualResultCode) {
        ...
        System.out
            .println("failed (exception was expected to be thrown during Phase " + (expectedResultCode + 1) + ")");
    ...
    // In the case that execution did not successfully complete when it should have,
    // output
    // a failure message
